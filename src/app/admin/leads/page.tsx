    ...
    <div style={styles.loginWrap}>
      <div style={{ ...styles.loginCard, animation: shaking ? 'shake 0.4s ease' : 'none' }}>
        ...
        <div style={styles.inputWrap}>
          <input
            type="password" placeholder="Enter password" value={value}
        ...
        <button onClick={submit} style={styles.loginBtn}>Access Dashboard →</button>
      </div>
      <style>{`
        @keyframes shake {
...
function DetailModal({
  lead, tab, updatingId, onClose, onUpdateStatus,
}: {
  lead: AdvisorLead | PackageLead;
  tab: TabType;
  ...
  const a = lead as AdvisorLead;
  const p = lead as PackageLead;

  const fields: [string, string][] = isAdvisor
    ? [
        ['Email',          a.email],
        ['Phone',          a.phone],
        ['Business Type',  a.business_type],
        ['Location',       a.location],
        ['Generator Size', a.generator_size || '—'],
        ['Monthly Fuel',   a.fuel_spend     || '—'],
        ['Monthly Grid',   a.grid_spend     || '—'],
        ['Notes',          a.message        || '—'],
        ['Source',         a.source         || '—'],
        ['IP Address',     a.ip             || '—'],
    ...
    <div style={styles.modalBackdrop} onClick={onClose}>
      <div style={{ ...styles.modal, width: 560 }} onClick={e => e.stopPropagation()}>
        <div style={styles.modalHeader}>
          <div>
            <h3 style={styles.modalName}>{lead.name}</h3>
            <span style={{
              fontSize: 11, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.08em',
              color: isAdvisor ? '#f59e0b' : '#a78bfa',
            }}>
              {isAdvisor ? 'Advisor Enquiry' : 'Package Enquiry'}
            </span>
          </div>
          <button onClick={onClose} style={styles.closeBtn}>✕</button>
        </div>

        <div style={{ ...styles.modalGrid, gridTemplateColumns: '1fr 1fr' }}>
          {fields.map(([label, value]) => (
            <div key={label} style={styles.modalField}>
              <span style={styles.modalLabel}>{label}</span>
              <span style={{ ...styles.modalValue, wordBreak: 'break-word' }}>{value}</span>
            </div>
          ))}
        </div>
          ...
          <div style={{ display: 'flex', gap: 8, marginTop: 10 }}>
            {(['new', 'contacted', 'qualified'] as const).map(s => (
              <button
                ...
                style={{
                  ...styles.statusBtn,
                  background: lead.status === s ? STATUS_CONFIG[s].bg    : 'rgba(255,255,255,0.05)',
                  color:      lead.status === s ? STATUS_CONFIG[s].color : '#94a3b8',
                  border:    `1px solid ${lead.status === s ? STATUS_CONFIG[s].color : 'transparent'}`,
                }}
              >
...
export default function LeadsDashboard() {
  ...
  const [tab,          setTab]          = useState<TabType>('advisor');
  const [leads,        setLeads]        = useState<(AdvisorLead | PackageLead)[]>([]);
  ...
  const [selected,     setSelected]     = useState<AdvisorLead | PackageLead | null>(null);
  ...
  const fetchLeads = useCallback(async () => {
    ...
    try {
      const params = new URLSearchParams({
        page: String(page), limit: '15', status: statusFilter,
        ...(search ? { search } : {}),
      });
      const res  = await fetch(`${API_MAP[tab]}?${params}`);
  ...
  const switchTab = (t: TabType) => {
    setTab(t); setPage(1); setSearch(''); setStatusFilter('all'); setSelected(null);
  };

  const updateStatus = async (id: string, status: string) => {
    setUpdatingId(id);
    await fetch(API_MAP[tab], {
      method: 'PATCH',
    ...
    await fetchLeads();
    if (selected?._id === id) {
      setSelected(prev => prev ? { ...prev, status: status as AdvisorLead['status'] } : null);
    }
  };

  if (!authed) return (
    <LoginScreen onLogin={() => { sessionStorage.setItem('leads_authed', 'true'); setAuthed(true); }} />
  );

  // Table columns per tab
  const advisorCols  = ['Name', 'Email', 'Phone', 'Business Type', 'Location', 'Status', 'Date', ''];
  const packageCols  = ['Name', 'Email', 'Phone', 'Package', 'System', 'Biz Type', 'Status', 'Date', ''];
    ...
    <div style={styles.shell}>
      ...
      <aside style={styles.sidebar}>
        ...
        <div style={{ padding: '16px 12px 8px', borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
          {(['advisor', 'package'] as const).map(t => (
            <button
              key={t}
              onClick={() => switchTab(t)}
              style={{
                ...
                borderLeft: `2px solid ${tab === t ? '#f59e0b' : 'transparent'}`,
                marginBottom: 2,
              }}
            >
              {t === 'advisor' ? '👤 Advisor Leads' : '📦 Package Enquiries'}
            </button>
          ))}
        </div>
          ...
          {(['all', 'new', 'contacted', 'qualified'] as const).map(f => (
            <button
              ...
              style={{
                ...
                borderLeft: `2px solid ${statusFilter === f ? '#f59e0b' : 'transparent'}`,
              }}
            >
        ...
        <button
          onClick={() => { sessionStorage.removeItem('leads_authed'); setAuthed(false); }}
      ...
      <main style={styles.main}>
        <header style={styles.header}>
          <div>
            <h2 style={styles.pageTitle}>
              {tab === 'advisor' ? 'Advisor Leads' : 'Package Enquiries'}
              {statusFilter !== 'all' && ` — ${STATUS_CONFIG[statusFilter as keyof typeof STATUS_CONFIG].label}`}
            </h2>
            {pagination && (
        ...
        <div style={styles.tableWrap}>
          {loading ? (
            ...
            <div style={styles.emptyState}><p style={{ color: '#475569' }}>No leads found.</p></div>
          ) : (
            <table style={styles.table}>
              <thead>
                <tr>
                  {(tab === 'advisor' ? advisorCols : packageCols).map(h => (
                ...
                {tab === 'advisor'
                  ? (leads as AdvisorLead[]).map(lead => (
                      ...
                      >
                        ...
                        <td style={styles.td}>
                          <span style={{ ...styles.badge, color: STATUS_CONFIG[lead.status]?.color, background: STATUS_CONFIG[lead.status]?.bg }}>
                            {STATUS_CONFIG[lead.status]?.label}
                          </span>
                        </td>
                        <td style={{ ...styles.td, whiteSpace: 'nowrap', fontSize: 12, color: '#64748b' }}>{formatDate(lead.createdAt)}</td>
                        <td style={styles.td}><button onClick={() => setSelected(lead)} style={styles.viewBtn}>View</button></td>
                      </tr>
                    ))
                  : (leads as PackageLead[]).map(lead => (
                      <tr key={lead._id} style={styles.tr}
                        onMouseEnter={e => (e.currentTarget.style.background = 'rgba(255,255,255,0.03)')}
                        onMouseLeave={e => (e.currentTarget.style.background = 'transparent')}
                      >
                        ...
                        <td style={styles.td}>{lead.email}</td>
                        <td style={styles.td}>{lead.phone}</td>
                        <td style={{ ...styles.td, color: '#a78bfa', fontWeight: 600 }}>{lead.package}</td>
                        <td style={styles.td}>{lead.system  || '—'}</td>
                        <td style={styles.td}>{lead.bizType || '—'}</td>
                        <td style={styles.td}>
                          <span style={{ ...styles.badge, color: STATUS_CONFIG[lead.status]?.color, background: STATUS_CONFIG[lead.status]?.bg }}>
                            {STATUS_CONFIG[lead.status]?.label}
                          </span>
...
const styles: Record<string, React.CSSProperties> = {
  ...
  statCard:    { background: 'rgba(255,255,255,0.04)', borderRadius: 10, padding: '12px 14px', display: 'flex', flexDirection: 'column', gap: 2 },
  ...
  navBtn:      { textAlign: 'left', padding: '10px 12px', borderRadius: 8, fontSize: 14, fontWeight: 500, cursor: 'pointer', border: 'none', transition: 'all 0.15s', width: '100%' },
  logoutBtn:   { margin: '0 16px', padding: '10px', background: 'transparent', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 8, color: '#475569', fontSize: 13, cursor: 'pointer' },
  main:        { flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' },
  header:      { display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '28px 32px 20px', borderBottom: '1px solid rgba(255,255,255,0.06)' },
  ...
  searchInput: { background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 10, padding: '10px 14px 10px 36px', color: '#e2e8f0', fontSize: 14, outline: 'none', width: 280 },
  tableWrap:   { flex: 1, overflowX: 'auto', overflowY: 'auto', padding: '0 32px' },
  table:       { width: '100%', borderCollapse: 'collapse', marginTop: 8 },
  th:          { textAlign: 'left', padding: '12px 14px', fontSize: 11, fontWeight: 600, color: '#475569', textTransform: 'uppercase', letterSpacing: '0.08em', borderBottom: '1px solid rgba(255,255,255,0.06)' },
  ...
  viewBtn:     { padding: '5px 14px', background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 6, color: '#94a3b8', fontSize: 13, cursor: 'pointer' },
  emptyState:  { display: 'flex', justifyContent: 'center', alignItems: 'center', height: 200 },
  spinner:     { width: 28, height: 28, border: '2px solid rgba(255,255,255,0.1)', borderTop: '2px solid #f59e0b', borderRadius: '50%', animation: 'spin 0.7s linear infinite' },
  paginationRow: { display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 20, padding: '20px 32px', borderTop: '1px solid rgba(255,255,255,0.06)' },
  pageBtn:     { padding: '8px 18px', background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 8, color: '#94a3b8', fontSize: 13, cursor: 'pointer' },
  modalBackdrop: { position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.7)', backdropFilter: 'blur(4px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 50 },
  modal:         { background: '#0f1829', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 16, padding: 32, width: 480, maxWidth: '90vw', maxHeight: '90vh', overflowY: 'auto' },
  ...
  statusBtn:     { padding: '8px 16px', borderRadius: 8, fontSize: 13, fontWeight: 600, cursor: 'pointer', transition: 'all 0.15s' },
};